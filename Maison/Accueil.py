    ...
    if prenomEntre.get() and nomEntre.get() and photo:
        ...
        if appartient(listePersonne, pn):
            ...
            showinfo(title="validation reussie",
                     message=prenomEntre.get() + " " + nomEntre.get() + " a bien été ajouter.")
    else:
...
b3 = Button(fen, text="Voir la liste ", command=lambda: listeInscrit(fen, listePersonne), width=10, font=fontLabel,
            bg='#FF7800', fg='white')
